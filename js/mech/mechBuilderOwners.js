function buildFullMechs(address){
    let fullMechs = {};
    RARITY_ORDER.forEach((model)=>{
        let min = 99999;
        // Find the lowest count part - taking arms in 2s
        PARTS_ORDER.forEach((part)=>{
            let count = parseInt(dataModel.owners[address].modelParts[model][part]);
            if(part == 'Arm'){
        ...
        for(let i=0; i<min; i++){
            if(dataModel.owners[address].remainingAfterglows > 0){
                ...
                Object.keys(dataModel.owners[address].modelParts[model]).forEach((part)=>{
                    if(part == 'Arm'){
                        dataModel.owners[address].modelParts[model][part]-=2;
                    }else {
...
function buildMixedMechs(address, afterglowRequired, allowPartial, allowNoModel){
    let mixedMechs = {};
    RARITY_ORDER.forEach((model)=>{
        let partCount = dataModel.owners[address].modelParts[model]['Engine'];
        if(allowNoModel){
            partCount = getMostParts(dataModel.owners[address].modelParts[model]);
        }
        for(let i=0; i< partCount; i++){
            ...
            remainingParts = changePartOrderBasedOnAvailability(address, model, remainingParts);
...
function changePartOrderBasedOnAvailability(address, modelType, remainingParts){
            ...
            if(modelType != model){
                PARTS_ORDER.forEach((part)=>{
                    if(!otherPartCountMax[part]){
                        otherPartCountMax[part] = dataModel.owners[address].modelParts[model][part];
                    } else {
                        otherPartCountMax[part] += dataModel.owners[address].modelParts[model][part];
                    }
                })
