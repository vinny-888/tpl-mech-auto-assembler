                    ...
                    if(allowNoModel || (partOne != '' && partTwo != '')){
                        ...
                        if(!allowNoModel || (allowNoModel && countMechParts(mixedMech) >= 2)){
                            if( (allowPartial && isPartialMech(mixedMech))
                                ...
                                // if(!afterglowRequired || (afterglowRequired && dataModel.owners[address].remainingAfterglows > 0) ){
                                    if(!mixedMechs[model]){
                                        mixedMechs[model] = [];
                                    }
...
function buildSameModelMechsStyles(address, afterglowRequired, allowPartial, allowNoModel){
    let mixedMechs = {};
    RARITY_ORDER.forEach((model)=>{
        ...
        if(dataModel.owners[address].modelParts[model]){
            ...
            for(let i=0; i< partCount; i++){
                ...
                if(allowNoModel || (partOne != '' && partTwo != '')){
                    ...
                    // }
                    if(!allowNoModel || (allowNoModel && countMechParts(mixedMech) >= 2)){
                        if( allowPartial || isFullMech(mixedMech)){ //&& isPartialMech(mixedMech))
                            // || (!allowPartial && isFullMech(mixedMech))){
                            // if(!afterglowRequired || (afterglowRequired && dataModel.owners[address].remainingAfterglows > 0) ){
                                if(!mixedMechs[model]){
                                    mixedMechs[model] = [];
...
function buildMixedModelMechsStyles(address, afterglowRequired, allowPartial, allowNoModel){
    let mixedMechs = {};
    let mixedMech = {};
    let hasEngine = false;
    ...
    RARITY_ORDER.forEach((model)=>{
        if(!remainingParts){
            remainingParts = Object.keys(tempRemainingParts[model]);
        }
        let order = sortByHighestCountFirst(model);
        ...
        if(dataModel.owners[address].modelParts[model]){
            ...
            for(let i=0; i< partCount; i++){
                    ...
                    order.forEach((style)=>{
                        if((part != 'Engine' || allowNoModel) 
                            ...
                            if(partOne == '' && tempRemainingParts[model][part][style] > 0){
                                partOne = part;
                                sameModel = model;
                                if(part == 'Arm' && tempRemainingParts[model][part][style] > 1 && sameModel == model){
                                    partTwo = part;
                                }
                            } else if(partTwo == ''  && tempRemainingParts[model][part][style] > 0){
                                partTwo = part;
                ...
                // Engine + 2 part available
                if(allowNoModel || (partOne != '' && partTwo != '' && engine != '')){
                    let mech = [
                        {
                            model: model,
                            part: engine
                    ...
                    // if(tempRemainingParts[model] && tempRemainingParts[model]['Engine'] && sameModel == model){
                    //     Object.keys(tempRemainingParts[model]['Engine']).forEach((style2)=>{
                    //         if(!hasEngine && tempRemainingParts[model] && tempRemainingParts[model]['Engine'] && tempRemainingParts[model]['Engine'][style2] && tempRemainingParts[model]['Engine'][style2] > 0){
                    //             mixedMech['Engine'] = {
                    //                 model,
                    ...
                    //     })
                    // }
                    let remainingPartNames = ['Engine', 'Head', 'Body', 'Leg', 'Arm', 'Arm'];
                    mech.forEach((modelPart)=>{
                        var index = remainingPartNames.indexOf(modelPart.part);
                        if (index !== -1) {
                            remainingPartNames.splice(index, 1);
                            if(modelPart.part == 'Engine' && !mixedMech['engine']){
                                let found = false;
                                order.forEach((style)=>{
                                    if(!found){
                                        if(tempRemainingParts[model]['Engine'][style] > 0){
                    ...
                    for(let j=0; j < remainingPartNames.length; j++){
                        ...
                        for(let i=0; i < orderRarityOrder.length; i++){
                            ...
                            order.forEach((style2)=>{
                                if(isPartialMech(mixedMech) && tempRemainingParts[model2] && tempRemainingParts[model2][part] && tempRemainingParts[model2][part][style2] && tempRemainingParts[model2][part][style2] > 0){
                                    // Still need a left arm
                                    if(part == 'Engine' && !mixedMech['Engine']){
                                        mixedMech['Engine'] = {
                                            model: model2,
                                            style: style2
                                        };
    ...
    if(!allowNoModel || (allowNoModel && countMechParts(mixedMech) >= 2)){
        if( allowPartial || isFullMech(mixedMech)){ //&& isPartialMech(mixedMech))
            // || (!allowPartial && isFullMech(mixedMech))){
            // if(!afterglowRequired || (afterglowRequired && dataModel.owners[address].remainingAfterglows > 0) ){
                if(!mixedMechs[sameModel]){
                    mixedMechs[sameModel] = [];
                }
                mixedMechs[sameModel].push(mixedMech);
                dataModel.owners[address].modelParts = tempRemainingParts;
